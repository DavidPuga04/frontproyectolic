...
axios.get(
`${API_URL}/api/tramites/top_sucursales_afluencia/`
)

.then(res => {
setTopSucursales(res.data);
})
...
if (tipo === 'sangre')
formData.append('archivo_sangre', archivo);

if (tipo === 'psico')
formData.append('archivo_psico', archivo);
...
if (!zona)
return alert("Debe seleccionar una zona");
...
{topSucursales.map((sucursal, index) => (
...
<h4>
#{index + 1} - {sucursal.sucursal}
</h4>

<p>
⏱ Espera promedio:
<strong>
{" "}{sucursal.espera_promedio} min
</strong>
</p>
...
if (paso === 1)
setArchivos({ ...archivos, cedula: file });

if (paso === 2)
setArchivos({ ...archivos, sangre: file });

if (paso === 3)
setArchivos({ ...archivos, psico: file });
}}
/>
